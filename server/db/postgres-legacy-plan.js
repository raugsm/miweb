...
function tableListForCounts({ includeMigrationRuns = true } = {}) {
...
export async function assertPostgresRequiredMigrations(client) {
  ...
  const missing = requiredMigrationVersions.filter((version) => !applied.has(version));
  if (missing.length) {
    const error = new Error(`Migraciones PostgreSQL requeridas pendientes: ${missing.join(", ")}`);
    error.code = "POSTGRES_REQUIRED_MIGRATIONS_MISSING";
    error.missingMigrations = missing;
    throw error;
  }
}

export function plannedPostgresTableCounts(plan, options = {}) {
  const tableSet = new Set(tableListForCounts(options));
  return Object.fromEntries(Object.entries(plan.tables || {}).filter(([table]) => tableSet.has(table)));
}
...
export async function applyPostgresLegacyRows(client, plan, { includeMigrationRuns = true } = {}) {
  await client.query("set local search_path = ariad, public");
  for (const table of insertOrder) {
    const insertRowsForTable = table === "customer_orders"
      ? plan.rows[table].map((row) => ({ ...row, frp_order_id: null }))
  ...
  if (includeMigrationRuns) await insertRows(client, "migration_runs", plan.rows.migration_runs);
  return queryPostgresRuntimeCounts(client, { includeMigrationRuns });
}
...
export async function replacePostgresLegacyRuntime(client, plan) {
  ...
  const mismatches = reportMismatches(expectedTables, actualTables);
  if (mismatches.length) {
    const error = new Error("Postgres runtime write produjo conteos inconsistentes.");
    error.mismatches = mismatches;
    throw error;
  }
  return { actualTables, expectedTables, mismatches };
}

export {
  collectionNames as POSTGRES_COLLECTION_NAMES,
  targetTables as POSTGRES_TARGET_TABLES,
  insertOrder as POSTGRES_INSERT_ORDER,
  columnsByTable as POSTGRES_COLUMNS_BY_TABLE,
  reportBlockPatterns as POSTGRES_REPORT_BLOCK_PATTERNS,
  requiredMigrationVersions as POSTGRES_REQUIRED_MIGRATIONS,
  sha256,
  buildImportPlan as buildPostgresLegacyPlan,
  findDuplicatePlannedPrimaryKeys,
  reportMismatches as reportPostgresCountMismatches,
  nonEmptyTables as nonEmptyPostgresTables,
  queryTargetCounts as queryPostgresTargetCounts,
  insertRows as insertPostgresRows,
  applyImport as applyPostgresImport,
  sanitizeErrorMessage as sanitizePostgresErrorMessage,
  ensureReportSafe as ensurePostgresReportSafe,
};
